  ...
  for nr_of_iteration in nrs_of_iterations:
    for nr_of_threads in threads_per_block:
      for blocks in blocks_per_grid:
        duration, cost_best_path = main('./graphs'+graph_path, nr_of_iteration, nr_of_threads, blocks)
        single_data = {"name":            str(graph_path)+'_iters'+str(nr_of_iteration)+'_threads'+str(nr_of_threads)+'_blocks'+str(blocks),
                      "graph":            graph_path,
